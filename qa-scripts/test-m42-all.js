...
async function testOverview(browser) {
...
async function testUnit(browser, unit) {
  const ctx = await browser.newContext({viewport:{width:820,height:1180},deviceScaleFactor:2});
  const page = await ctx.newPage();
  const errs = [];
  const issues = [];
  ...
  console.log(`[INFO] Homework: ${checks.homework}`);
  console.log(`[INFO] Resources: ${checks.resources}`);
  console.log(`[INFO] Vitamin: ${checks.vitamin}`);
  console.log(`[INFO] Next teaser: ${checks.nextTeaser}`);
  console.log(`[INFO] Recap: ${checks.recap}`);
  console.log(`[INFO] Stick/SVG: ${checks.stickFigure}`);
...
(async()=>{
  ...
  // Overview
  const overviewScore = await testOverview(browser);
  allResults.push({unit:'overview', score:overviewScore});

  // Units
  for(const u of UNITS){
    const r = await testUnit(browser, u);
    allResults.push(r);
  }
